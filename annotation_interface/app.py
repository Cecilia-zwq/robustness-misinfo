...
def public_response_item(raw):
    ...
    return {
        ...
        # Long-text beliefs: `belief` is the headline, `belief_long_text` the
        # passage the user actually read. The header renders them differently.
        "belief_is_long_text": bool(raw.get("is_long_text")),
        "belief_long_text": raw.get("long_text", "") or "",
...
def public_conversation(raw):
    ...
    payload = {
        ...
        # See public_response_item — same headline / passage split.
        "belief_is_long_text": bool(belief.get("is_long_text")),
        "belief_long_text": belief.get("long_text", "") or "",
        "n_turns": len(turns),
        "turns": turns,
        "blind": config.BLIND_METADATA,
    }
...
def api_batch():
    ...
    # A mode is a (task, unit) pair: rating-in-context, rating-isolated, or the
    # qualitative pass. The first two write ratings, the third writes codes.
    ...
    if mode == "response":
        ...
        for it in batch["items"]:
            st = status.get(it["item_id"],
                            {"status": "not_started", "n_responses": 0,
                             "rated_turns": []})
            items.append({
                "unit_id": it["item_id"],
                ...
                "rated_turns": st.get("rated_turns", []),
            })
        return jsonify({"batch": name, "mode": mode, "annotator": annotator,
                        "blind": config.BLIND_METADATA, "items": items})

    # Both conversation-scoped modes read the same batch of sessions; only the
    # unit_type they look up progress under differs.
    unit_type = "qualitative" if mode == "qualitative" else "conversation"
    batch = load_batch(name)
    if batch is None:
        return jsonify({"error": f"batch '{name}' not found"}), 404
    sids = batch["session_ids"]
    status = store.status_map(annotator, unit_type, sids) if annotator else {}
    items = []
    for sid in sids:
...
def api_conversation(session_id):
    annotator = request.args.get("annotator", "").strip()
    # The same transcript backs two independent records: the per-response
    # ratings and the qualitative coding pass. Never mix them.
    unit_type = request.args.get("unit_type", "conversation")
    if unit_type not in ("conversation", "qualitative"):
        return jsonify({"error": "unit_type must be conversation or qualitative"}), 400
    raw = load_conversation_file(session_id)
    if raw is None:
        return jsonify({"error": "conversation not found"}), 404
    annotation = store.load(annotator, unit_type, session_id) if annotator else None
    return jsonify({
...
def api_save_annotation():
    ...
    if not record.get("annotator_id") or not record.get("unit_id") \
            or unit_type not in ("conversation", "response", "qualitative"):
