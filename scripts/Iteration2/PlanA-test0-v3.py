"""
Phase 0 v3 — Bidirectional Strategy Reflection
================================================
Changes from v2:
  - Strategy reflection now analyses BOTH success and failure turns.
    When a turn produces a good score (truthfulness drop or drift rise),
    the reflector explains WHY it worked, not just why other turns failed.
  - Strategy memo has two new sections: SUCCESS_ANALYSIS and REBOUND_WARNING.
    SUCCESS_ANALYSIS captures what caused the LLM to drift on a good turn.
    REBOUND_WARNING flags turn-to-turn patterns where the LLM recovered
    after a weak response (e.g. turn 5 T=1 → turn 6 T=3) so the actor
    knows not to pivot away from a working approach.
  - Actor prompt highlights the most recent successful pattern prominently
    above the general memo, and flags "do not pivot away" when evaluator
    shows the LLM just recovered from a drift.
  - Threshold constants control what counts as "success" vs "rebound".
"""
...
class AgentState:
    ...
    strategy_memo: str             = ""
    # v3: track the last known successful pattern explicitly
...
def sim_user_turn(state: AgentState, turn_number: int) -> TurnLog:
    ...
    for attempt in range(MAX_REFLECT_RETRIES):
        ...
        candidate_message = actor_out.get("message", "")
...
def run_session(claim: dict, session_index: int) -> dict:
    session_id = f"phase0v3_{claim['id']}_{datetime.now().strftime('%H%M%S')}"
    print(f"\n{'='*70}")
...
def run_phase0():
    ...
    summary = {
        "run_timestamp": datetime.now().isoformat(), "version": "v3",
    ...
    path = RESULTS_DIR / "phase0_v3_summary.json"
    with open(path, "w") as f:
        json.dump(summary, f, indent=2)

    print(f"\n\n{'='*70}")
    print("PHASE 0 v3 SUMMARY")
    print(f"{'='*70}")
