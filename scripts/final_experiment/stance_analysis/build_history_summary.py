"""
stance_analysis/build_history_summary.py
==========================================
Join per-turn correction/rebuttal/agreeableness scores
(scores/<session_id>__misinfo_v1_split.json, the three-independent-
prompt rubric) with per-turn history-aware stance scores
(scores/<session_id>__misinfo_stance_history_split.json, produced by
run_stance_history_scoring.py) into one long CSV, restricted to the
sampled sessions from select_history_sample.py.

Uses misinfo_v1_split rather than misinfo_v1 (turn_level.csv) for
correction/rebuttal/agreeableness — see build_summary.py's module
docstring for why (the two rubrics disagree non-trivially on Claude's
agreeableness trend in particular).

Output: config.HISTORY_PER_TURN_OUTPUT_PATH, one row per (session_id, turn):
...
correction/rebuttal/agreeableness (-1.0) are dropped entirely — same
rule the reference notebook (final_experiment_analysis.ipynb) uses for
its misinfo_v1_split-based figures.
...
def _load_stance_scores(session_id: str) -> dict[int, float]:
    path = cfg.SCORES_DIR / f"{session_id}__{cfg.HISTORY_STANCE_RUBRIC_NAME}.json"
    if not path.exists():
        return {}
    with path.open("r", encoding="utf-8") as fh:
        data = json.load(fh)
...
def main() -> None:
    if not cfg.HISTORY_SAMPLE_INDEX_PATH.exists():
        raise SystemExit(
            f"Sample index not found: {cfg.HISTORY_SAMPLE_INDEX_PATH}\n"
            "Run `python -m stance_analysis.select_history_sample` first."
        )

    with cfg.HISTORY_SAMPLE_INDEX_PATH.open("r", encoding="utf-8") as fh:
    ...
    if n_missing_v1_split:
        print(
            f"Warning: {n_missing_v1_split} sampled session(s) missing a "
            f"{V1_SPLIT_RUBRIC_NAME} score file — skipped entirely."
        )
    if n_missing_stance:
        print(
            f"Warning: {n_missing_stance} sampled session(s) missing a history-stance "
            f"score file — run `python -m stance_analysis.run_stance_history_scoring` "
            f"first (or re-run it to fill in the gap)."
        )
    ...
    if not rows:
        raise SystemExit(
            "No sampled session produced any usable rows — run "
            "`python -m stance_analysis.run_stance_history_scoring` first."
        )
    out = pd.DataFrame(rows, columns=columns).sort_values(["session_id", "turn"])
    cfg.HISTORY_PER_TURN_OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    out.to_csv(cfg.HISTORY_PER_TURN_OUTPUT_PATH, index=False)
    print(
        f"Wrote {len(out):,} rows ({out['session_id'].nunique():,} sessions) "
        f"→ {cfg.HISTORY_PER_TURN_OUTPUT_PATH}"
    )
