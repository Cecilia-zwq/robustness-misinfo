...
class TurnRecord:
    ...
    belief_breaks : int
        Number of belief breaks detected by reflection on this turn.
    is_fallback : bool
...
class SessionResult:
    ...
    n_character_breaks_total : int
        Total number of character breaks flagged by the reflection module
        across all turns.
    n_belief_breaks_total : int
        Total number of belief breaks flagged by the reflection module
        across all turns.
    ...
    n_character_breaks_total: int = 0
    n_belief_breaks_total: int = 0
    ...
    def mean_correction_all(self) -> float:
        ...
        vals = [
            t.scores.get("correction")
            for t in self.turns
            if t.scores.get("correction") is not None
            and t.scores.get("correction") != -1
        ]
        return sum(vals) / len(vals) if vals else 0.0
    ...
    def mean_rebuttal_all(self) -> float:
        ...
        vals = [
            t.scores.get("rebuttal")
            for t in self.turns
            if t.scores.get("rebuttal") is not None
            and t.scores.get("rebuttal") != -1
        ]
        return sum(vals) / len(vals) if vals else 0.0
    ...
    @property
    def belief_break_rate(self) -> float:
        """Fraction of all turns where at least one belief break occurred."""
        if not self.turns:
            return 0.0
        return sum(1 for t in self.turns if t.belief_breaks > 0) / len(self.turns)
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "n_character_breaks_total": self.n_character_breaks_total,
            "n_belief_breaks_total": self.n_belief_breaks_total,
            "n_breaks_fallback": self.n_breaks_fallback,
            "character_break_rate": round(self.character_break_rate, 3),
            "belief_break_rate": round(self.belief_break_rate, 3),
...
def run_session(
    ...
    for turn_idx in range(1, n_turns + 1):
        ...
        char_breaks = user_agent._last_character_break_count
        belief_breaks = user_agent._last_belief_break_count
        is_fallback = user_agent._last_fallback

        # Accumulate session-level break counters
        result.n_character_breaks_total += char_breaks
        result.n_belief_breaks_total += belief_breaks
        if is_fallback:
        ...
        logger.info(
            ...
            "char_breaks: %d, belief_breaks: %d, is_fallback: %s",
        ...
        result.turns.append(
            TurnRecord(
                ...
                scores=scores,
                character_breaks=char_breaks,
                belief_breaks=belief_breaks,
